...
const expresiones = {
    ...
    expr2: /^[A-Za-záéíóúÁÉÍÓÚüÜñÑ]{2,50}$/,
...
function validateForm(event) {
    ...
    if (!expresiones.expr2.test(RazaMascota.value)) {
        document.getElementById('Error2').textContent = 'Debes ingresa una raza valida, Verifique.';
    ...
    if (!expresiones.expr3.test(ColorMascota.value)) {
        document.getElementById('Error3').textContent = 'Debes ingresar un color valido, Verifique.';
    ...
    if (!file || !imageTypes.includes(file.type)) {
        document.getElementById('Error4').textContent = 'Debes ingresar una foto valida (Jpeg, png, jpg), no puede it sin foto, Verifique.';
